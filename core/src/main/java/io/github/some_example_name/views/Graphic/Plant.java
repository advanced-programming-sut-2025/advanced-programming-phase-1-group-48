...
public enum State {
    ...
    public void stateIn() {
        ...
        if (stageIndex < thresholds.length) {
            if(stageIndex == 0) {
                growthTexture = new Texture(Gdx.files.internal("plants/Crops/" +cropType.getName().toLowerCase()+"_seeds" + ".png"));
            }else{
                growthTexture = new Texture(
                    Gdx.files.internal("plants/Crops/" +cropType.getName().toLowerCase()+"_stage_"+ stageIndex + ".png")
                );
            }
        } else {
            // بالغ
